...
public class ElementName {
    ...
    public void getMap(){
        ...
        for (int i=0; i<chainLen-1; i+=2){
            try{
                baseChain[i].bondTo(baseChain[i+1], baseBonds[i], "R");
            } catch(Exception e) {
            }
        }
        ...
        for (int i=0; i<l; i++){
            ...
            for (int j=0; j<n.length; j++){
                Element next = new Element(0, 0, name);
                baseChain[n[j]].bondTo(next, "single", "U");
            }
        }
